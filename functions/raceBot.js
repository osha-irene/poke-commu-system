...
// feedNunmegiRace) gameData/nunmegiRace의 1~5번 누니머기 중 한 마리가 무작위로 1mm씩
...
const formatRaceStatus = (raceState) => {
  ...
  const racerLines = RACER_IDS.map((id, index) => {
    const line = `${id}번 누니머기는 ${progressOf(id)}mm`;
    return index === RACER_IDS.length - 1 ? `${line} 을 가고 있는 것 같다!` : line;
  });
