// 커뮤니티 자체 콘테스트 규칙표 "포켓몬 콘테스트 기술 및 콤보" 시트의 콤보표 기준 - src/contest/comboChart.js를
// 그대로 포팅한 CommonJS 버전.
// (선행 기술 id -> [{ moveId: 연계 기술 id, bonusAppeal: 추가 획득 하트, bonusJam: 추가 방해 수치 }])
// bonusAppeal은 콤보 성공 시 연계 기술 사용자의 어필에 추가되고,
// bonusJam은 콤보 성공 시 GM/봇이 지정한 대상(targetId)에게 추가로 적용되는 방해 수치다.
const COMBO_CHART = {
  'belly-drum': [
...
const isComboStarter = (moveId) => !!COMBO_CHART[moveId];

const getComboBonus = (starterMoveId, followUpMoveId) =>
  COMBO_CHART[starterMoveId]?.find((c) => c.moveId === followUpMoveId) || null;

const isValidComboFollowUp = (starterMoveId, followUpMoveId) =>
  !!getComboBonus(starterMoveId, followUpMoveId);

module.exports = { COMBO_CHART, isComboStarter, getComboBonus, isValidComboFollowUp };
